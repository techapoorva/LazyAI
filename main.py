...
""" gRPC client for Google Assistant API."""
...
WORKDIR = '~/'
audio_response = 'okgoogle.wav'
wavep = wave.open(audio_response, 'rb')

class SampleAssistant(object):
    """
      conversation_stream =  audio stream
        for recording query and playing back to assistant as answer.
      channel: authorized gRPC channel for connection.
    ...
    def converse(self):
        """Sending voice requests to the Assistant and getting response.

        Returns: True if conversation should continue.
        """
        continue_conversation = False

        # have to start playback first before writing into output stream
        '''
        self.conversation_stream.start_playback()
        self.conversation_stream.write(wavep.readframes(100000))
...
def main(api_endpoint, credentials, verbose,
    ...
    with SampleAssistant(conversation_stream,
        ...
        # read audio files for trigger response
        samples = []
        ...
        while True:
            # check if bluetooth device (e.g. AB Shutter 3) connected
            BT_device = get_BT_device_list()

            if BT_device:
                while True:
                    BT_event = False # flag for trigger key recognised
                    if wait_for_user_trigger:
                        # flush key events before going into next read_loop()
                        while BT_device.read_one() != None:
                            pass
                        BT_event = wait_for_BT_trigger(BT_device)
                        # break while loop if BT device disconnected
                        if not BT_event:
                            break
                ...
                time.sleep(5) # pause for 5 seconds before next device check
